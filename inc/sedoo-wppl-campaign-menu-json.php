 ...
 */
function sedoo_campaign_wp_api_v2_menus_get_all_menus()
{
	...
	return apply_filters('sedoo_campaign_wp_api_v2_menus__menus', $menus);
}
...
function sedoo_campaign_wp_api_v2_menu_get_all_locations()
{
	...
	return apply_filters('sedoo_campaign_wp_api_v2_menus__locations', $locations);
}
 ...
 */
function sedoo_campaign_wp_api_v2_locations_get_menu_data($data)
{
	...
	if (($locations = get_nav_menu_locations()) && isset($locations[$data['id']])) {
		...
		$menu->items = sedoo_campaign_wp_api_v2_menus_get_menu_items($locations[$data['id']]);
	} else {
	...
	return apply_filters('sedoo_campaign_wp_api_v2_menus__menu', $menu);
}
 ...
 */
function sedoo_campaign_wp_api_v2_menus_dna_test(&$parents, $child)
{
	foreach ($parents as $key => $item) {
		...
		if ($item->child_items) {
			if (sedoo_campaign_wp_api_v2_menus_dna_test($item->child_items, $child)) {
				return true;
			}
		}
	}
 ...
 */
function sedoo_campaign_wp_api_v2_find_object_by_id($array, $id)
{
 ...
 */
function sedoo_campaign_wp_api_v2_menus_get_menu_items($id)
{
	...
	do {
		foreach ($child_items as $key => $child_item) {
			$parent = sedoo_campaign_wp_api_v2_find_object_by_id($all_menu_items, $child_item->menu_item_parent);
			if (empty($parent)) {
				unset($child_items[$key]);
			} else if (sedoo_campaign_wp_api_v2_menus_dna_test($menu_items, $child_item)) {
				unset($child_items[$key]);
			}
		}
	} while (count($child_items));
	return apply_filters('sedoo_campaign_wp_api_v2_menus__menu_items', array_values($menu_items));
}
 ...
 * @return object Menu's data with his items
 */
function sedoo_campaign_wp_api_v2_menus_get_menu_data($data)
{
	// This ensure retro compatibility with versions `<= 0.5` when this endpoint
	//   was allowing locations id in place of menus id
	if (has_nav_menu($data['id'])) {
		$menu = sedoo_campaign_wp_api_v2_locations_get_menu_data($data);
		...
		$menu        = get_term($id);
		$menu->items = sedoo_campaign_wp_api_v2_menus_get_menu_items($id);
	} else {
...
add_action('rest_api_init', function () {
	register_rest_route('menus/v1', '/menus', array(
		'methods'  => 'GET',
		'callback' => 'sedoo_campaign_wp_api_v2_menus_get_all_menus',
		'permission_callback' => '__return_true'
	));

	register_rest_route('menus/v1', '/menus/(?P<id>[a-zA-Z0-9_-]+)', array(
		'methods'  => 'GET',
		'callback' => 'sedoo_campaign_wp_api_v2_menus_get_menu_data',
		'permission_callback' => '__return_true'
	));

	register_rest_route('menus/v1', '/locations/(?P<id>[a-zA-Z0-9_-]+)', array(
		'methods'  => 'GET',
		'callback' => 'sedoo_campaign_wp_api_v2_locations_get_menu_data',
		'permission_callback' => '__return_true'
	));

	register_rest_route('menus/v1', '/locations', array(
		'methods'  => 'GET',
		'callback' => 'sedoo_campaign_wp_api_v2_menu_get_all_locations',
		'permission_callback' => '__return_true'
	));
});
