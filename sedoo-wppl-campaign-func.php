...
function sedoo_campaign_init_create_catalogue() {
    if(!get_field('id_composant_catalogue', 'option')) {
        ...
        $scripts_values = array(
			...
			array("script"   => '<script>  document.ssoAerisInitOptions = {url : "https://sso.aeris-data.fr/auth",     realm : "aeris",     clientId : "$$CAMPAIGNNAME$$-vjs",     resource: "catalogue-aeris-services",  authorizedDomains: ["https://api.sedoo.fr"]    } </script>'),
