const BASE_URL = "https://api.sedoo.fr/sedoo-campaigns-rest";
...
jQuery("#CreateBackEnd").click(function () {
  ...
  jQuery.ajax({
    url: BASE_URL + "/campaign/v1_0/findbyname/" + campaign_name,
    type: "GET",
    success: function (result) {
      if (result == "") {
        // si backend non existant je le crée
        jQuery.ajax({
          url: BASE_URL + "/campaign/v1_0/save",
          type: "POST",
      ...
      jQuery("#CreateBackEnd").after(
        '<a class="button button-primary nextstpbtn" href="admin.php?page=sedoo-campaign-admin-main-page">Etape suivante</a>'
      );
    }
  });
});
...
jQuery("#SynchroniseProducts").click(function () {
  ...
  var i = 0;
  jQuery.ajax({
    url: BASE_URL + "/inputproduct/v1_0/list/" + id_backend,
    type: "GET",
    success: function (result) {
      for (i; i < result.length; i++) {
        sedoo_campaign_createOrUpdateCampaignProduct(
          result[i],
          ajaxurl,
          i,
          result.length
...
function sedoo_campaign_createOrUpdateCampaignProduct(
  product,
  ajaxurl,
  i,
  total
  ...
  jQuery.ajax({
    ...
    success: function (result) {
      jQuery("#SynchroniseProducts").text(
        i + 1 + "/" + total + " Synchronisé(s) !"
      );
    }
  });
}
